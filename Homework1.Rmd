...
numbers <- NULL
```
This line of code assigns the value NULL to the variable called numbers. Nothing is printed because the value is being stored in a variable. When used with the below chunk of code, this line of code serves the purpose of initializing the numbers variable with an empty value. The variable is now available to have its value changed in the for loop.
...
# Split into control and treatment groups, and re-calculate avgs.
# treatment_samples <- setdiff(sample_cols, control_samples)
# ref_cols <- setdiff(names(array_data), sample_cols)
# 
# control_data <- array_data[c(ref_cols, control_samples)]
# control_data$AVG_EXPRESSION <- rowMeans(control_data[c(control_samples)], na.rm=T)
# trt_data <- array_data[c(ref_cols, treatment_samples)]
# trt_data$AVG_EXPRESSION <- rowMeans(trt_data[c(treatment_samples)], na.rm=T)
```

Histogram:

```{r hist by gene}
hist(array_data$AVG_EXPRESSION, main="Average Expression by Gene", xlab="Average Gene Expression Level", ylab="# of Genes")
```
...
https://www.ncbi.nlm.nih.gov/geo/geo2r/?acc=GSE764

TODO: go back and add descriptions to all manipulations and plots

## Question 4

Objective: For all of the columns in the UCI machine learning data set, calculate the number of missing values, the mean, median, standard deviation. For categorical variables, calculate the counts for each feature. 

QUESTION:
Visualize these results as boxplots (what results? All or just the categorical?). Describe any findings and why the calculated values may be different between each column. (Am I overthinking this? Each column is a different variable, so obviously the calculated variables are different between each column)
...
for (col in names(ml_data)){
  print(paste0("Statistical summary for column ", col, " :"))
  # Handle ? values in ca column
  ml_data[[col]] <- as.numeric(ml_data[[col]])

  # Calculate stats
  num_missing <- na_counts[[col]]
  print(paste0("Number of missing values: ", num_missing))
